...
def write_csv(info_book, name_category):
    file_csv = str(name_category) + '.csv'
    with open(file_csv, 'w', newline='', encoding='UTF-8') as files_csv:
        write = csv.DictWriter(files_csv, fieldnames=titles)
        write.writeheader()
        # print('voici >>',  info_book)
        for info_books in info_book:
            write.writerow({'product_page_url': info_books['product_page'],
                                 'universal_product_code': info_books['universal_code'],
                                 'title': info_books['title'],
                                 'price_including_tax': info_books['Price_including_tax'].strip(),
                                 'price_excluding_tax': info_books['price_excluding_tax'].strip(),
                                 'number_available': info_books['number_available'].strip(),
                                 'product_description': str(info_books['description']),
                                 'category': info_books['category'].strip(),
                                 'review_rating': info_books['review_Rating'].strip(),
                                 'image_url': info_books['image_url']})
